...
from data_loader import (
    ...
    get_historico_habilidades  # Assegure-se de que essa função existe em seu data_loader.py
)
...
# Função para adicionar ou atualizar colaboradores
def adicionar_colaborador():
    ...
    if funcoes:
        # Remove duplicatas de colaboradores (mesmo ID e nome)
        colaboradores_unicos = {}
        for colaborador in get_colaboradores():
            colaborador_id, nome = colaborador[0], colaborador[1]
            colaboradores_unicos[colaborador_id] = nome
    ...
    f"{id} - {nome}" for id, nome in sorted(colaboradores_unicos.items(), key=lambda item: item[1].lower())
        ...
        colaborador_selecionado = st.selectbox("Selecione um Colaborador para Atualizar ou Excluir", options=opcoes_colaboradores)
        ...
        if colaborador_selecionado != "Adicionar Novo Colaborador":
            ...
            colaborador_nome = next((nome for id, nome, _, _ in get_colaboradores() if id == colaborador_id), None)

            for f in funcoes:
                nivel_atual = next((nivel for id, _, funcao, nivel in get_colaboradores() if id == colaborador_id and funcao == f), None)
            ...
            if st.button(f"Atualizar Colaborador '{colaborador_nome}'"):
                atualizar_colaborador_db(colaborador_id, habilidades)
                st.success(f"Colaborador '{colaborador_nome}' atualizado com sucesso!")

            if st.button(f"Excluir Colaborador '{colaborador_nome}'"):
                excluir_colaborador_db(colaborador_id)
                st.success(f"Colaborador '{colaborador_nome}' excluído com sucesso!")
        else:
            nome_colaborador = st.text_input("Nome do Novo Colaborador")
            if nome_colaborador:
                for f in funcoes:
                    nivel = st.selectbox(f"Nível em '{f}' (0-5)", options=[0, 1, 2, 3, 4, 5], key=f"{nome_colaborador}_{f}")
                    habilidades[f] = nivel

                if st.button("Adicionar Novo Colaborador"):
                    adicionar_colaborador_db(nome_colaborador, habilidades)
                    st.success(f"Colaborador '{nome_colaborador}' adicionado com sucesso!")
    else:
        st.warning("Adicione pelo menos uma função antes.")

# Função para exibir a matriz de polivalência
def exibir_matriz():
